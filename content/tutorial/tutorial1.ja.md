...
## 1. 概要
泥質片岩の顕微鏡動画を用いて NicolNavi の基本操作を習得するための取り扱い説明書です。記載された手順を順に実施することで、ステージ回転動画の前処理、粒界抽出、鉱物ラベルの設定、モード組成および形状定向配列の確認までを一通り完了できます。
...
- 画面中央部：`Image list`左画面に表示する画面を選べます。
...
3. 左画面に動画の先頭フレームが 幅1000 px 四方の画像として表示されることを確認します。
...
2. 中心がずれている場合は `＋` `−` ボタンで座標を微調整します(今回の例では、ほとんどずれていないので、これはスキップ可能です)。
3. 調整後、`3 Continue` を押して回転中心を確定します

### 4.4 解析結果の確認
1. 自動遷移後の画面で `Image list` を開き、以下の代表的な解析マップを確認します。
   - `color`：ステージ 1 回転の中で最も明るい瞬間の色情報
   - `extinction color`：最も暗くなる瞬間の色情報（完全な消光が確認できない場合は前工程を確認してください）
   - `R`：各ピクセルのレタデーション値（石英程度のレタデーション以外は信頼できません）。
   - `extinction angle`：各ピクセルの光消角マップ
...
### 4.5 粒界抽出パラメータの調整
1. 右画面のパラメータ欄を確認し、目的に合わせて初期値を設定します。
2. `6 Detect Grain Boundary` を押し、左画面に粒界線が重ね表示されることを確認します。
2. `4 median filter`, `5 parmeters` を下図のように調整してください。
3. `6 Calculate grain boundaries` を再度押して更新します。
...
3. パラメータ欄の `8 Label Name` に `other minerals`を追加してください。

<img src="/images/page/tutorial/tutorial1/pic7.png" style="width: 100%;">

4. パラメータ欄の `10 mica`を選択後、左画面から当該鉱物をクリックしてください。5-10個程度追加します。
...
{{< notice info >}}
ラベル付与にはラベル伝搬法を使用しています。全粒子数のおよそ 20% 以上を教師データとして指定すると、分類結果が得られます。今回の例では、144粒子のラベル付を行いました。
{{< /notice >}}
